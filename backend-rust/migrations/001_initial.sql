-- Initial database schema for Traditional Chinese Study app
-- Creating all tables with UUID primary keys and proper relationships
...
CREATE INDEX idx_classics_slug ON classics(slug);
CREATE INDEX idx_chapters_classic_id ON chapters(classic_id);
CREATE INDEX idx_chapters_number ON chapters(number);
CREATE INDEX idx_sentences_chapter_id ON sentences(chapter_id);
CREATE INDEX idx_sentences_number ON sentences(number);
CREATE INDEX idx_characters_character ON characters(character);
CREATE INDEX idx_characters_frequency ON characters(frequency_rank);
...
CREATE INDEX idx_character_progress_user_id ON character_progress(user_id);
CREATE INDEX idx_character_progress_next_review ON character_progress(next_review);
