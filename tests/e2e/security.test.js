...
describe('E2E Security Tests', () => {
  describe('CSRF Protection', () => {
    test('should prevent CSRF attacks on all POST endpoints', async () => {
      const endpoints = ['/auth/register', '/auth/login', '/auth/logout', '/auth/children'];

      for (const endpoint of endpoints) {
        const response = await request(app).post(endpoint).send({ test: 'data' }).expect(403);
    ...
    test('should use secure session configuration', async () => {
      ...
      if (cookies) {
        const sessionCookie = cookies.find(
          cookie => cookie.includes('connect.sid') || cookie.includes('session')
        );
